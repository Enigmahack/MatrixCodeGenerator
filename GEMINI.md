...
1.  **Monolithic HTML:** A single self-contained file (e.g., `MatrixCode_v7.4.html`) for easy distribution.
2.  **Modular Directory:** A structured development environment (e.g., `MatrixCode_v7.4/`) with separate CSS and JS files.
...
*   **Split (Monolith -> Modular):**
    ...
    ```
    *Example:* `python3 matrix_builder.py split MatrixCode_v7.4.html MatrixCode_dev`
...
1.  **Edit:** Make changes in the modular directory (`MatrixCode_v7.4/`).
2.  **Test:** Open `MatrixCode_v7.4/index.html` in a browser to verify changes.
3.  **New Files:** If you add a new JS file (e.g., a new Effect):
    *   Place it in the appropriate subdirectory (e.g., `js/effects/`).
    *   Run `python3 matrix_builder.py refresh MatrixCode_v7.4`.

class ClearPulseEffect extends AbstractEffect {
    ...
    trigger() {
        ...
        // Snapshot only for filling gaps (Reveal Effect). 
        // We do NOT snapshot active chars so they can keep flowing.
    ...
    getOverride(i) {
        ...
        if (s.clearPulseBlend && !isGap) {
            ...
            const mR = Math.floor(tRgb.r + (baseRgb.r - tRgb.r) * rel);
            const mG = Math.floor(tRgb.g + (baseRgb.g - tRgb.g) * rel);
            const mB = Math.floor(tRgb.b + (baseRgb.b - tRgb.b) * rel);

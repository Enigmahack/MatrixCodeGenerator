    ...
    _u(prog, name) {
        ...
        _initShaders() {
            ...
            const lineFS = `#version 300 es
                ...
                float getLayerVal(vec4 occ, int layerIdx) {

                    if (layerIdx == 0) return occ.r;

                    if (layerIdx == 1) return occ.g;

                    if (layerIdx == 2) return occ.b; if (layerIdx == 3) return occ.a;
                ...
                void main() {
                    ...
                    vec2 nearestI = round(logicPos);

                    vec2 p = (logicPos - nearestI) * u_cellPitch; // Offset from intersection point
                    ...
                    int L3 = u_layerOrder[3];



                    // Sample 4 blocks meeting at nearestI
                    ...
                    for (int layerIdx = 0; layerIdx < 4; layerIdx++) {
                        ...
                        if (abs(aNW - aNE) > 0.01) {
                            ...
                            float line = 1.0 - smoothstep(halfThick - u_sharpness, halfThick + u_sharpness, d);

                            float glow = exp(-d * u_glowFalloff) * (u_glow * 0.5);

                            float val = max(line, glow) * max(aNW, aNE);

                            if (isL1 && a0NW > 0.01 && a0NE > 0.01) fadeMax = max(fadeMax, val);

                            else normalMax = max(normalMax, val);
                        ...
                        if (abs(aSW - aSE) > 0.01) {
                            ...
                            float val = max(line, glow) * max(aSW, aSE);

                            if (isL1 && a0SW > 0.01 && a0SE > 0.01) fadeMax = max(fadeMax, val);
                        ...
                        if (abs(aNW - aSW) > 0.01) {
                            ...
                            float val = max(line, glow) * max(aNW, aSW);

                            if (isL1 && a0NW > 0.01 && a0SW > 0.01) fadeMax = max(fadeMax, val);
                        ...
                        if (abs(aNE - aSE) > 0.01) {
                            ...
                            float val = max(line, glow) * max(aNE, aSE);

                            if (isL1 && a0NE > 0.01 && a0SE > 0.01) fadeMax = max(fadeMax, val);
                    ...
                    normalMax *= (1.0 - u_persistence);

                    fadeMax *= (1.0 - u_persistence);

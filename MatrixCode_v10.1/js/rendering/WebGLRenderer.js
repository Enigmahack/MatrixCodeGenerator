...
`;
            ...
            const matrixFS = `#version 300 es
                ...
                void main() {
                    ...
                    if (useMix >= 5.0 && u_shadowEnabled > 0.5) {
                        ...
                        finalAlpha = owA + tex2 * sFade * clampedSAlpha;
    ...
    render(frame) {
        ...
        // Get glassBloom for shadow alpha floor clamping
        let cpuGlassBloom = 1.0;
        if (fx) {
            const cpuFxState = fx.getWebGLRenderState(this.config.state, this.config.derived);
            if (cpuFxState) cpuGlassBloom = cpuFxState.glassBloom;
        }
        const sAlphaFloorCPU = 1.0 / Math.max(cpuGlassBloom, 1.001);

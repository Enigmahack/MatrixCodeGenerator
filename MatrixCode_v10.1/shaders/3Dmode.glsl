...
const float STRIP_CHAR_HEIGHT = 0.45;
const float STRIP_CHAR_WIDTH = 0.30;
const float ZCELL_SIZE = 0.4 * (STRIP_CHAR_HEIGHT * STRIP_CHARS_MAX);
...
float sample_glyph(float charIdx, vec2 uv) {
    ...
    // uv is 0..1 within the character cell
    // We need to map this to the atlas texture
...
float random_char(vec2 outer, vec2 inner, float highlight) {
    // Pick a random character from the atlas based on 'outer' seed
    float seed = hash(outer);
    float charIdx = floor(seed * u_charCount);
    float a = sample_glyph(charIdx, inner);
    return a + highlight * smoothstep(0.4, 0.0, 1.0 - a); // highlight effect
}
...
void main() {
    ...
    if (t8 < 0.125) {
        ...
        rotate_on_turns = 0.0; roll_on_turns = 0.0;
    } else {
        prev += dir + dir + dir;
        turn = dirR;
        turn_sign = 1.0;

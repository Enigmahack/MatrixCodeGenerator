class WebGLRenderer {
    ...
    _initShaders() {
        ...
        const bloomFS = `${version}
            ...
            uniform float u_opacity; // Added Opacity Control
            ${outColor}
            ...
            void main() {
                ...
                ${setFragColor} = vec4(result * u_opacity, 1.0); // Apply Opacity
            }
        `;
        ...
        if (!this.isWebGL2) {
             finalBloomFS = `
                ...
                void main() {
                    ...
                    gl_FragColor = vec4(result * u_opacity, 1.0);
                }
             `;
        }
    ...
    updateSmoothing() {
        // Apply CSS blur to match CanvasRenderer behavior
    ...
    _drawFullscreenTexture(texture, opacity, blurAmt) {
        ...
        this.gl.uniform1f(this.gl.getUniformLocation(this.bloomProgram, 'u_opacity'), opacity); // Set Opacity
    ...
    render(frame) {
        ...
        for (let fIdx = 0; fIdx < activeFonts.length; fIdx++) {
            ...
            const countCell = (i) => {
                ...
                if (effectiveFontName !== fontData.name) return;
                ...
                if (gridAlpha > 0.01) {
                    nMain++;
                    if (override && override.glow && override.glow > 0) nMain++; // GLOW INSTANCE RESERVATION
                ...
                if (tState.phase !== 'none' && tState.alpha > 0.01) {
                    nTracer++;
                    if (s.tracerGlow > 0) nTracer++; // GLOW INSTANCE RESERVATION
                }
            };
            ...
            for (const i of grid.activeIndices) {
                ...
                if (override) {
                     ...
                     if (override.solid) {
                         ...
                         // If solid, we are done (Pulse gaps/waiting). 
                         continue;
                     }
                     
                     // If not solid, it's a character override (Pulse Wave/Tracer Snapshot)
                     if (override.char && override.alpha > 0.01) {
                         let r=0, g=1, b=0;
                         if (override.color) {
                             const c = Utils.hexToRgb(override.color);
                             r = c.r/255; g = c.g/255; b = c.b/255;
                         } else {
                             // Fallback to palette/calc color if missing (shouldn't happen with Pulse)
                             r = paletteNorm[pIdx][0]; g = paletteNorm[pIdx][1]; b = paletteNorm[pIdx][2];
                         }

                         const charStr = override.char;
                         const sprite = atlas.get(charStr);
                         
                         // GLOW INSTANCE (Draw first for layering)
                         if (override.glow && override.glow > 0 && sprite) {
                             ...
                             const glowScale = drawScale + (Math.min(5, override.glow) / s.fontSize);
                             
                             this.instanceData[ptrMain++] = px;
                             this.instanceData[ptrMain++] = py;
                             this.instanceData[ptrMain++] = charIdx;
                             this.instanceData[ptrMain++] = r; this.instanceData[ptrMain++] = g; this.instanceData[ptrMain++] = b;
                             this.instanceData[ptrMain++] = override.alpha * 0.4;
                             this.instanceData[ptrMain++] = glowScale;
                         }
                         ...
                         if (sprite) {
                             ...
                             this.instanceData[ptrMain++] = charIdx;
                             this.instanceData[ptrMain++] = r; this.instanceData[ptrMain++] = g; this.instanceData[ptrMain++] = b;
                             this.instanceData[ptrMain++] = override.alpha;
                         ...
                         // Strictly skip dynamic logic (Tracer, Overlap, Ghost) for this cell
                         continue;
                     }
                }
                ...
                if (gridAlpha > 0.01) {
                    ...
                    // GLOW EFFECT (SIMULATED SHADOW BLUR)
                    if (override && override.glow && override.glow > 0) {
                        ...
                        if (sprite) {
                             ...
                             const row = Math.round(sprite.y / atlas.cellSize);
                             const charIdx = (row * atlas._lastCols) + col;
                             // Glow is approx: bigger scale, lower alpha
                             // Canvas shadowBlur is pixels. 20px blur on 25px font ~ 80% scale increase? No, blur extends edges.
                             // Approximation: Scale += glow/20. Alpha = 0.3.
                             const glowScale = drawScale + (Math.min(5, override.glow) / s.fontSize);
                             ...
                             this.instanceData[ptrMain++] = charIdx;
                             this.instanceData[ptrMain++] = r; this.instanceData[ptrMain++] = g; this.instanceData[ptrMain++] = b;
                             this.instanceData[ptrMain++] = mainAlpha * 0.4; // Softer backing
                             this.instanceData[ptrMain++] = glowScale;
                        } else {
                             // Dummy to keep stride
                             this.instanceData[ptrMain++] = px; this.instanceData[ptrMain++] = py; this.instanceData[ptrMain++] = -1;
                             this.instanceData[ptrMain++] = 0; this.instanceData[ptrMain++] = 0; this.instanceData[ptrMain++] = 0;
                             this.instanceData[ptrMain++] = 0; this.instanceData[ptrMain++] = 0;
                        }
                ...
                if (tState.phase !== 'none' && tState.alpha > 0.01) {
                     ...
                     if (sprite) {
                         ...
                         // TRACER GLOW (SIMULATED SHADOW BLUR)
                         if (s.tracerGlow > 0) {
                             const glowScale = (1.0 + (s.tracerSizeIncrease / s.fontSize)) + (Math.min(5, s.tracerGlow) / s.fontSize);
                             this.instanceData[ptrTracer++] = px;
                             this.instanceData[ptrTracer++] = py;
                             this.instanceData[ptrTracer++] = charIdx;
                             this.instanceData[ptrTracer++] = tracerColor[0]; this.instanceData[ptrTracer++] = tracerColor[1]; this.instanceData[ptrTracer++] = tracerColor[2];
                             this.instanceData[ptrTracer++] = tState.alpha * 0.4;
                             this.instanceData[ptrTracer++] = glowScale; 
                         }
        ...
        if (s.enableBloom) {
            ...
            // Apply s.bloomOpacity here in the final pass
